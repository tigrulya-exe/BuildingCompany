...
class BuildingObjectService(
    ...
    fun findByManagementsOrWorkTypes(
        filter: ObjectsByWorkTypeQueryDto?,
        params: FindRequestParameters?
    ): Page<BuildingObject> {
        val pageable = getPageable(params)
        return objectRepository.findByManagementsOrWorkTypes(
            getNullableList(filter?.managementIds),
            filter?.startDateMin,
            filter?.startDateMax,
            getNullableList(filter?.workTypeIds),
            pageable
        )
    }
